...
<details>
<summary>2021.11.16 ~ 17(나현)</summary>

## 구현한 것

- 기술 검색 부분의 UI/UX
  - 기술 태그를 선택했을 때와 선택을 취소했을 때를 구현
    예를 들면, 버튼 하나씩 눌렀을 때 맨 앞에 추가되고, 기존에 버튼을 누르지 않은 목록들 재정렬해서 선택한 버튼은 안보이게 함.
    반대로 선택을 취소했을 때는 기존 선택된 기술 태그 목록에서 제외시키고, 선택하지 않은 태그 목록의 원래 위치로 돌아가게 함.
  - 선택하지 않은 기술 태그들은 처음에 15개만 보이게 함.
    또한 기술 태그를 선택하더라도 선택하지 않은 기술들은 15개로 계속 고정되어 보이게 함.
  - 기술 태그 더보기/접기 구현
  - 기술 검색 폼을 열고 닫는 기능 구현

## 앞으로 진행할 작업

- 성능 최적화를 위해 기술 검색 부분 컴포넌트 분리
- 선택한 기술들을 url 쿼리에 추가
- redux-saga 부분을 구현하여 기술 검색 시 임의로 서버 통신하는 것 처럼 더미 데이터 로드하기
